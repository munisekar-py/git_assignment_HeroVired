...
```
git remote add origin https://github.com/munisekar-py/git_assignment_HeroVired.git
...
gh pr create --base main --head dev --title "Bug fix divide function merge dev into main" --body "Implemented bug fix in divide function on dev branch requested Abhijit to review the code and provide approval to merge dev branch with main" --reviewer abhijeetchavan9243
```
...
git checkout dev
git status
gh pr create --base dev --head feature/sqrt --title "Implement sqrt feature and merge feature/sqrt into dev" --body "Implemented sqrt feature requesting Abhijit to review and approve it so that feature/sqrt branch can be merged into dev" --reviewer abhijeetchavan9243
```
